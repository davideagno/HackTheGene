...
# column mean and sd
cmeans_train <- as.numeric(colMeans(counts_train, na.rm = TRUE))
csds_train <- as.numeric(matrixStats::colSds(as.matrix(counts_train), na.rm = TRUE))

# centered, rescaled and normalized train matrices
centered_counts_train <- counts_train
rescaled_counts_train <- counts_train
normalized_counts_train <- counts_train
for (j in 1:n) {
  centered_counts_train[,j] <- centered_counts_train[,j] - cmeans_train[j]
  rescaled_counts_train[,j] <- rescaled_counts_train[,j] / csds_train[j]
  normalized_counts_train[,j] <- (normalized_counts_train[,j] - cmeans_train[j]) / csds_train[j]
} 

# Cosine similarities (rescaled = original, normalized = centered)
csims_train <- matrix(0, nrow = n, ncol = n)
centered_csims_train <- matrix(0, nrow = n, ncol = n)
for(i in 1:n) {
  for (j in i:n) {
    ...
    # centered
    nas_train <- unique(sort(c(na_train[[i]], na_train[[j]])))
    a <- centered_counts_train[-nas_train,i]
    b <- centered_counts_train[-nas_train,j]
    centered_csims_train[i,j] <- cosine(a,b)
    centered_csims_train[j,i] <- centered_csims_train[i,j]
  }
}
...
mse_normalized_counts <- rep(0, length(K_val))
...
for (index in 1:length(K_val)) {
  ...
  top_centered_csims_train <- apply(centered_csims_train, 2, function(x) order(x, decreasing = TRUE)[1:(K+1)])
  
  final_counts_train <- counts_train
  final_centered_counts_train <- centered_counts_train
  final_rescaled_counts_train <- rescaled_counts_train
  final_normalized_counts_train <- normalized_counts_train
  ...
  for (j in 1:n) {
    ...
    nearest_centered_users_train <- top_centered_csims_train[-1,j]
    near_centered_values_train <- final_centered_counts_train[na_train[[j]], nearest_centered_users_train]
    near_normalized_values_train <- final_normalized_counts_train[na_train[[j]], nearest_centered_users_train]
    weight_centered_csims_train <- centered_csims_train[nearest_centered_users_train, j]
    ...
    for (i in 1:length(na_train[[j]])) {
      ...
      final_normalized_counts_train[na_train[[j]][i],j] <- weighted.mean(near_normalized_values_train[i,], weight_centered_csims_train, na.rm = TRUE)
    }
  ...
  for (j in 1:n) {
    ...
    final_normalized_counts_train[,j] <- final_normalized_counts_train[,j] * csds_train[j] + cmeans_train[j]
    final_normalized_counts_train <- pmax(as.matrix(final_normalized_counts_train), 0)
  ...
  final_normalized_counts_train[,c(284, 1062, 1246, 2312)] <- 0
  error_nas <- which(is.na(final_normalized_counts_train), arr.ind = TRUE)
  if(dim(error_nas)[1] > 0)
  {
    print('norm')
    print(dim(error_nas)[1])
    for (i in 1:dim(error_nas)[1]) {
      final_normalized_counts_train[error_nas[i,1],error_nas[i,2]] <- mean(final_normalized_counts_train[error_nas[i,1],][!is.na(final_normalized_counts_train[error_nas[i,1],])])
    }
  }
  
  print('done error')
  out_orig <- final_counts_train[ind_val]
  out_norm <- final_normalized_counts_train[ind_val]
  mse_counts[index] <- mse(out_orig, answers_valid)
  mse_normalized_counts[index] <- mse(out_norm, answers_valid)
  
}

cbind(K_val, mse_counts, mse_normalized_counts)
plot(K_val, mse_counts, type = "l")
lines(K_val, mse_normalized_counts, col = 2)
